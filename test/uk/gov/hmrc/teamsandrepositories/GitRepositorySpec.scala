...
class GitRepositorySpec extends WordSpec with Matchers with OptionValues {
  ...
  "repoGroupToRepositoryDetails" should {
    ...
    "create links for a closed service" in {
      ...
      val repos = Seq(GitRepository("a-frontend", "Some Description", "https://not-open-github/org/a-frontend", now, now, isInternal = true, repoType = RepoType.Service, updateDate = System.currentTimeMillis()))
    ...
    "create links for a closed libraries" in {
      ...
      val repos = Seq(GitRepository("a-library", "Some Description", "https://not-open-github/org/a-library", now, now, isInternal = true, repoType = RepoType.Library, updateDate = System.currentTimeMillis()))
    ...
    "create links for a open Libraries" in {
      ...
      val repo = Seq(GitRepository("a-library", "Some Description", "https://github.com/org/a-library", now, now, repoType = RepoType.Library, updateDate = System.currentTimeMillis()))
    ...
    "create links for a open service" in {
      ...
      val repo = Seq(GitRepository("a-frontend", "Some Description", "https://github.com/org/a-frontend", now, now, repoType = RepoType.Service, updateDate = System.currentTimeMillis()))
    ...
    "create links for private libraries on github.com" in {
      ...
      val repo = Seq(GitRepository("a-library", "Some Description", "https://github.com/org/a-library", now, now, isPrivate = true, repoType = RepoType.Library, updateDate = System.currentTimeMillis()))
    ...
    "create links for private services on github.com" in {
      ...
      val repo = Seq(GitRepository("a-frontend", "Some Description", "https://github.com/org/a-frontend", now, now, isPrivate = true, repoType = RepoType.Service, updateDate = System.currentTimeMillis()))
    ...
    "create links for each environment" in {
      val aFrontend = GitRepository("a-frontend", "Some Description", "https://not-open-github/org/a-frontend", now, now, repoType = RepoType.Service, updateDate = System.currentTimeMillis())
    ...
    "do not create environment links for libraries" in {
      val aLibrary = GitRepository("a-library", "Some Description", "https://not-open-github/org/a-library", now, now, repoType = RepoType.Library, updateDate = System.currentTimeMillis())
    ...
    "create github links for both open and internal services if both are present, but only open ci links" in {
      ...
      val internalRepo = GitRepository("a-frontend", "Some Description", "https://not-open-github/org/a-frontend", now, now, isInternal = true, repoType = RepoType.Service, updateDate = System.currentTimeMillis())

      val openRepo = GitRepository("a-frontend", "Some Description", "https://github.com/org/a-frontend", now, now, repoType = RepoType.Service, updateDate = System.currentTimeMillis())
    ...
    "create github links for both open and internal Libraries if both are present, but only ci-open links" in {
      ...
      val internalRepo = GitRepository("a-library", "Some Description", "https://not-open-github/org/a-library", now, now, isInternal = true, repoType = RepoType.Library, updateDate = System.currentTimeMillis())

      val openRepo = GitRepository("a-library", "Some Description", "https://github.com/org/a-library", now, now, repoType = RepoType.Library, updateDate = System.currentTimeMillis())
    ...
    "just create github links if not Deployable or Library" in {
      ...
      val internalRepo = GitRepository("a-repo", "Some Description", "https://not-open-github/org/a-repo", now, now, isInternal = true, repoType = RepoType.Other, updateDate = System.currentTimeMillis())

      val openRepo = GitRepository("a-repo", "Some Description", "https://github.com/org/a-repo", now, now, repoType = RepoType.Other, updateDate = System.currentTimeMillis())
    ...
    "read an object without the isPrivate field" in {
      ...
      GitRepository.gitRepositoryFormats.reads(
        ...
        """
          ...
          |"isInternal":true,
          |"repoType":"Other",
          |"updateDate":123}""".stripMargin)
      ).get shouldBe GitRepository("a-repo", "Some Description", "https://not-open-github/org/a-repo", 1499417808270L, 1499417808270L, isInternal = true, repoType = RepoType.Other, updateDate = 123l)
