...
class GithubV3RepositoryDataSourceSpec extends WordSpec with ScalaFutures with Matchers with DefaultPatienceConfig with MockitoSugar with SpanSugar with BeforeAndAfterEach {
  ...
  "Github v3 Data Source " should {
    ...
    "Set type Service if the repository contains a deploy.properties" in new Setup {
      ...
      dataSource.mapTeam(org, team).futureValue shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, repoType = RepoType.Service, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "extract digital service name from repository.yaml" in new Setup {
      ...
      dataSource.mapTeam(org, team).futureValue shouldBe TeamRepositories("A", List(GitRepository("repository-xyz", "some description", "url_A", now, now, repoType = RepoType.Other, digitalServiceName = Some("service-abcd"), updateDate = timestampF())), timestampF())
    }
    ...
    "extract digital service name and repo type from repository.yaml" in new Setup {
      ...
      dataSource.mapTeam(org, team).futureValue shouldBe TeamRepositories("A", List(GitRepository("repository-xyz", "some description", "url_A", now, now, repoType = RepoType.Service, digitalServiceName = Some("service-abcd"), updateDate = timestampF())), timestampF())
    }
    ...
    "Set type as Library according if the repository.yaml contains a type of 'library'" in new Setup {
      ...
      dataSource.mapTeam(org, team).futureValue shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, repoType = RepoType.Library, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "Set type as Other if the repository.yaml contains any other value for type" in new Setup {
      ...
      dataSource.mapTeam(org, team).futureValue shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, repoType = RepoType.Other, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "Set type Library if not Service and has src/main/scala and has tags" in new Setup {
      ...
      repositories shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, repoType = RepoType.Library, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "Set type Prototype if the repository name ends in '-prototype'" in new Setup {
      ...
      val repositories = dataSource.mapTeam(org, team).futureValue
      repositories shouldBe TeamRepositories("A", List(GitRepository("CATO-prototype", "some description", "url_A", now, now, repoType = RepoType.Prototype, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "Set type Other if not Service, Library nor Prototype and no repository.yaml file" in new Setup {
      ...
      val repositories = dataSource.mapTeam(org, team).futureValue
      repositories shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, repoType = RepoType.Other, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
    ...
    "Set isPrivate to true if the repo is private" in new Setup {
      ...
      repositories shouldBe TeamRepositories("A", List(GitRepository("A_r", "some description", "url_A", now, now, isPrivate = true, repoType = RepoType.Other, digitalServiceName = None, updateDate = timestampF())), timestampF())
    }
